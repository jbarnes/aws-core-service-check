...
Thanks for your interest in improving aws-core-service-check. Issues and pull
requests are welcome.

## Development setup

Requires Python 3.9 or higher. AWS credentials are only needed to run the tool
against a real organization; the test suite mocks all AWS calls.
...
- CI must pass on all supported Python versions (3.9-3.14). This runs both the
...
This tool is strictly read-only against AWS. It must never create, modify, or
delete AWS resources. Remediation steps are documented in the README for the
operator to run deliberately, but the script itself only reads.
